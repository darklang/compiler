...
### Affine induction strength reduction

- Optimization name: Affine induction strength reduction
- Taxonomy category: Loop optimization
- Priority/rationale: Carrying loop-invariant `a * i + b` values removes a repeated multiply and affine add/subtract from every hot `i + 1` iteration.
- Notes: Implemented in MIR for a deliberately narrow two-block, single-backedge `Int64` loop. The preheader computes the initial affine value using the incoming values of invariant phis, a header phi carries it, and the latch advances it by `a` after its last use. The `derived_induction_affine_*` MIR snapshots cover positive, negative, overflow, reused-value, and noncanonical-step shapes; `tailcall.e2e` covers wrapping execution. The existing ARM64 quick Leibniz result remains 1,000,143 to 900,145 instructions (10.0%) for the canonical form; the generalized form has a deterministic MIR reduction of one multiply per iteration.
