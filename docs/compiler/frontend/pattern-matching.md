...
List structure must always be traversed through the typed private list
operations before tuple fields are projected from an element. Treating a list
node itself as a tuple once caused list-of-tuples patterns to read structural fields
as user values. The public regression coverage lives in the list-pattern and
`List.zip` E2E cases.
