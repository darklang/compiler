...
| Sorting and uniqueness | `sort`, `sortBy`, `unique`, and `uniqueBy` use canonical typed ordering. `sortByComparator` is stable, accepts an `Int` comparator, and returns the exact error unless each result is `-1`, `0`, or `1`. The pinned interpreter's order-preserving `dedup` remains unimplemented. |
