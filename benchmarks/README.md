...
multi-file applications use a release Cargo profile with `opt-level = 3` and a
single standardized `benchmark` binary. Dark is built with its
...
contains 21 pairs. It excludes incomparable nsieve and the reduced Dark
...
Every problem has one `dark/main.dark` and one `rust/main.rs`. The quick profile
invokes those same sources with reduced argv workloads declared in
`profiles.json`. `quick_check.sh` builds the current Dark compiler, compiles Rust with
...
benchmarks/
  ...
  problems/
    fib/                     # Each benchmark has its own directory
      dark/main.dark         # Parameterized Dark implementation
      rust/main.rs           # Parameterized Rust implementation or Cargo driver
...
3. Add each profile's positional arguments and expected stdout to `profiles.json`.
...
- **Output validation**: Every execution is checked against its `profiles.json` expected stdout
