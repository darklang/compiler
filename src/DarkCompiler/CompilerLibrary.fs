...
let private normalizeMirFunctionRegisterOffset
    (func: MIR.Function)
    : MIR.Function =
    let minimumRegisterId =
        ...
        let includeInstr minimum instr =
            let minimum =
                match MIR_Optimize.getInstrDest instr with
                | Some dest -> includeReg minimum dest
                | None -> minimum
            let minimum =
                MIR_Optimize.foldInstrUses includeReg minimum instr
            match instr with
        ...
        |> Map.fold (fun minimum _ block ->
            ...
            MIR_Optimize.foldTerminatorUses includeReg minimum block.Terminator) minimum
...
type private MirFunctionNameHashComparer() =
...
type MirOptimizationKey = {
    ...
    Options: MIR_Optimize.OptimizeOptions
    EffectFreeCalls: Set<string>
}

type private MirOptimizationKeyNameHashComparer() =
...
type private Arm64InstructionChunkGroupReferenceComparer() =
        ...
        if shouldRunMIROptimize mirOptions then
            ...
            let effectFreeFunctions =
                if mirOptions.EnableLICM || mirOptions.EnableCSE then
                    MIR_Optimize.analyzeEffectFreeFunctions
