// AST_to_ANF.fs - ANF Transformation (Pass 2)
//
...
/// Typed comparison marker emitted by TypeChecking. It remains in generic
...
let private int128Construction (value: System.Int128) : ANF.CExpr =
...
let private uint128Construction (value: System.UInt128) : ANF.CExpr =
...
let private int128LiteralComparison (valueAtom: ANF.Atom) (value: System.Int128) : ANF.CExpr =
    let (low, high) = int128Words value
    ANF.Call ("Stdlib.Int128.__equalsWords", [valueAtom; ANF.IntLiteral (ANF.UInt64 low); ANF.IntLiteral (ANF.UInt64 high)])

let private uint128LiteralComparison (valueAtom: ANF.Atom) (value: System.UInt128) : ANF.CExpr =
    let (low, high) = uint128Words value
...
let private firstDeclaredRecordFields
    (fields: (string * AST.Type) list)
    : (string * AST.Type) list =
    fields
...
let private buildDeclaredRecordFieldSubst
...
type private LambdaComparisonPlan = {
...
let private planLambdaComparison
    (parameters: AST.NonEmptyList<AST.LambdaParameter>)
    (body: AST.Expr)
    (state: LiftState)
...
let private makeClosureComparator
...
let private buildSkewListLiteral
    (listType: AST.Type)
