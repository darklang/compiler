...
let private translateInstr (instr: LIR.Instr) : Result<X86_64.Instr list, string> =
    ...
    | LIR.PrintInt64 reg ->
        // For now, just print the number and newline using write syscall
        // This is a placeholder — a real implementation needs itoa
        resolveReg reg
        |> Result.map (fun _srcReg ->
            // TODO: implement integer-to-string conversion
            // For now, just exit with the value as exit code
            [])
