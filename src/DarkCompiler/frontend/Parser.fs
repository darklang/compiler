    ...
    and parsePostfix (expr: Expr) (toks: Token list) : Result<Expr * Token list, string> =
        ...
        | TAdjacentLParen :: rest ->
            ...
            | Constructor _, _ ->
                ...
                // parse the parenthesized fields as their tuple payload, here
                // rather than as a space application, so that in argument
                // position `f Type.Variant(a, b)` passes one argument to `f`.
                parsePrimaryBase (TLParen :: rest)
                |> Result.bind (fun (payload, afterPayload) ->
                    parsePostfix (appendCallArg expr payload) afterPayload)
